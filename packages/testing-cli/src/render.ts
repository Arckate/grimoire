...
interface Render {
	...
	debugStdin: (escapeAnsi?: boolean) => void;
}
...
export async function render({
	...
	setup,
	user,
	options: { delay = 20, enableLogTerm = false, enableLogStdin = false } = {
		delay: 20,
		enableLogTerm: false,
		enableLogStdin: false,
	},
	...
	const mockStdout = Object.create(process.stdout) as typeof process.stdout;
	...
	const mockStderr = Object.create(process.stderr) as typeof process.stderr;
	...
	const mockStdin = getStdin(seed);
	if (enableLogStdin) mockStdin.enableLog();
	const stdin = mockStdin as unknown as typeof process.stdin;
	...
	const finish = (): Render => {
		...
		return {
			...
			debugStdin: debugStdinTest(originalStderrWrite, mockStdin.getLogger()),
		};
	};
