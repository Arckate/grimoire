...
import { CONF_INITS, DEFAULT_INIT, INIT_PLOP_FILE } from "@arckate/grimoire-core/const";

beforeEach(() => {
	vi.resetAllMocks();

	vi.spyOn(pathModule, "findPlopPath").mockReturnValue(
		"mocked/plop/path.js",
	);
	...
	vi.spyOn(pathModule, "pathConstructor").mockResolvedValue(
		"mocked/output",
	);
...
describe("cmdInit", () => {
	...
	it("should catch error and exit 1", async () => {
		...
		expect(utilsModule.formatError).toHaveBeenCalledWith(
			expect.any(Error),
		);
