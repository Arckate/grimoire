		...
		const constructorStep = async (
			...
			switch (type) {
				...
				case STARTER_TYPES.MULTI_CHOICE: {
					...
					const currentOtherKey = step.otherKey? step.otherKey.map((key) => `${nameSpace}${nameSpace ? capitalizeFirst(key): key}`): [];
					...
					if ([STARTER_TYPES.FILTER, STARTER_TYPES.UNKNOWN].includes(typeChild)) {
						...
						if (value && typeof values[value] === "string") {
							[currentKey, ...currentOtherKey].forEach(key => innerFilters[key] = values[key])
							break;
						}
						[currentKey, ...currentOtherKey].forEach(key => innerFilters[key] = value)
				...
				case STARTER_TYPES.FILTER: {
					const valueFilter = filterValues<
						CmdPlopParams | FilePlopParams | StarterLink | CmdInitParams | SetValues<CmdPlopParams | FilePlopParams | CmdInitParams>
					>(step, innerFilters);
