...
import { ProcessTerm } from "../entities/ProcessTerm";
...
export const findFilePlopArgs = async ({
	...
	console.log('ette1')
	const { isValidPath, isDirectory, isFile } = await pathValidating(processTerm, toGenFiles);
	console.log('ette2', isValidPath)
	if (isValidPath) {
